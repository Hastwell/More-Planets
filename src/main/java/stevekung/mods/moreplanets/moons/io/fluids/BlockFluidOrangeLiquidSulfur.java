...
public class BlockFluidOrangeLiquidSulfur extends BlockFluidLavaBaseMP
{
    public BlockFluidOrangeLiquidSulfur(String name)
    {
        super(IoBlocks.orange_liquid_sulfur_fluid);
    ...
    public void randomDisplayTick(World world, BlockPos pos, IBlockState state, Random rand)
    {
        if (this.blockMaterial == Material.lava && world.getBlockState(pos.up()).getBlock().getMaterial() == Material.air && !world.getBlockState(pos.up()).getBlock().isOpaqueCube())
        {
            if (rand.nextInt(100) == 0)
            {
                ...
                MorePlanetsCore.proxy.spawnParticle(EnumParticleTypesMP.ORANGE_SULFUR_LAVA, d5, d6, d7);
