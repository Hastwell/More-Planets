...
public class RoomBossIo extends DungeonRoom
{
    ...
    public RoomBossIo(MapGenDungeon dungeon, int x, int y, int z, EnumFacing facing)
    {
        ...
        for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
        {
            for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
            {
                for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
                {
                    ...
                    {
                        this.placeBlock(chunk, i, j, k, cx, cz, MarsBlocks.creeperEgg, 0);
                    }
                    else
                    {
    ...
    {
        return new RoomBossIo(dungeon, x, y, z, facing);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        ...
        {
            TileEntityDungeonSpawner spawner = new TileEntityIoDungeonSpawner();
