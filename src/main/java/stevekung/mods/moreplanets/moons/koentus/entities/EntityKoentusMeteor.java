    ...
    protected void onImpact(MovingObjectPosition moving)
    {
        if (!this.worldObj.isRemote)
        {
            if (moving != null)
            {
                ...
                {
                    this.worldObj.setBlock(moving.blockX, moving.blockY + 1, moving.blockZ, KoentusBlocks.fallen_koentus_meteor, 0, 3);
                }
                if (moving.entityHit != null)
                {
                    moving.entityHit.attackEntityFrom(EntityKoentusMeteor.causeMeteorDamage(this, this.shootingEntity), ConfigManagerCore.hardMode ? 12F : 6F);
                }
            }
