...
import stevekung.mods.moreplanets.moons.deimos.blocks.BlockDeimos;
import stevekung.mods.moreplanets.moons.deimos.blocks.DeimosBlocks;
import stevekung.mods.moreplanets.planets.diona.entities.EntityEvolvedEnderman;

public class ChunkProviderDeimos extends ChunkProviderGenerate
{
	IBlockState topBlock = DeimosBlocks.deimos_block.getDefaultState();
	IBlockState fillBlock = DeimosBlocks.deimos_block.getDefaultState().withProperty(BlockDeimos.VARIANT, BlockDeimos.BlockType.deimos_sub_surface_rock);
	IBlockState lowerBlock = DeimosBlocks.deimos_block.getDefaultState().withProperty(BlockDeimos.VARIANT, BlockDeimos.BlockType.deimos_rock);
	...
	public void generateTerrain(int chunkX, int chunkZ, ChunkPrimer chunk)
	{
		...
		for (int x = 0; x < ChunkProviderDeimos.CHUNK_SIZE_X; x++)
		{
			for (int z = 0; z < ChunkProviderDeimos.CHUNK_SIZE_Z; z++)
			{
				...
				for (int y = 0; y < ChunkProviderDeimos.CHUNK_SIZE_Y; y++)
				{
					if (y < ChunkProviderDeimos.MID_HEIGHT + yDev)
					{
		...
		for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
		{
			for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
			{
				for (int x = 0; x < ChunkProviderDeimos.CHUNK_SIZE_X; x++)
				{
					for (int z = 0; z < ChunkProviderDeimos.CHUNK_SIZE_Z; z++)
					{
						if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderDeimos.CHUNK_SIZE_X + x, cz * ChunkProviderDeimos.CHUNK_SIZE_Z + z) / ChunkProviderDeimos.CRATER_PROB)
						{
