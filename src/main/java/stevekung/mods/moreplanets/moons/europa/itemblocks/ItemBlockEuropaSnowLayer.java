...
public class ItemBlockEuropaSnowLayer extends ItemBlockMorePlanets
{
	public ItemBlockEuropaSnowLayer(Block block)
	{
		super(block);
	}
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != EuropaBlocks.europa_snow_layer || (Integer)state.getValue(BlockSnowLayerMP.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}