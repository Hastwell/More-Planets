...
public class RoomBossPluto extends DungeonRoom
{
	...
	public RoomBossPluto(MapGenDungeon dungeon, int x, int y, int z, EnumFacing facing)
	{
			...
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					if (i == this.posX - 1 || i == this.posX + this.sizeX || j == this.posY - 1 || j == this.posY + this.sizeY || k == this.posZ - 1 || k == this.posZ + this.sizeZ)
					{
						...
						{
							this.placeBlock(chunk, i, j, k, cx, cz, PlutoBlocks.xeonium_glowstone, 0);
						}
						else
						{
	...
	{
		return new RoomBossPluto(dungeon, x, y, z, facing);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		if (tile == null || !(tile instanceof TileEntityPlutoDungeonSpawner))
		{
			TileEntityDungeonSpawner spawner = new TileEntityPlutoDungeonSpawner();
