...
import stevekung.mods.moreplanets.planets.pluto.worldgen.dungeon.RoomBossPluto;
import stevekung.mods.moreplanets.planets.pluto.worldgen.dungeon.RoomChestsPluto;
import stevekung.mods.moreplanets.planets.pluto.worldgen.dungeon.RoomTreasurePluto;

public class ChunkProviderPluto extends ChunkProviderGenerate
{
    Block topBlockID = PlutoBlocks.pluto_block;
    byte topBlockMeta = 0;
    Block fillBlockID = PlutoBlocks.pluto_block;
    byte fillBlockMeta = 1;
    Block lowerBlockID = PlutoBlocks.pluto_block;
    ...
    public ChunkProviderPluto(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < ChunkProviderPluto.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < ChunkProviderPluto.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < ChunkProviderPluto.CHUNK_SIZE_Y; y++)
                {
    ...
    public void populate(IChunkProvider par1IChunkProvider, int par2, int par3)
    {
        ...
        for (int i = 0; i < 8; ++i)
        {
            ...
            new WorldGenSpaceDungeons(PlutoBlocks.pluto_ancient_chest, PlutoBlocks.pluto_block, MPBlocks.space_mossy_cobblestone, 8).generate(this.worldObj, this.rand, x1, y1, z1);
        }
