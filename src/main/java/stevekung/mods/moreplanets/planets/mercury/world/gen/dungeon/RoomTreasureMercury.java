...
public class RoomTreasureMercury extends DungeonRoom
{
	...
	public RoomTreasureMercury(MapGenDungeon dungeon, int x, int y, int z, EnumFacing facing)
	{
	...
	public void generate(Block[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, MercuryBlocks.mercury_treasure_chest, 0))
		{
	...
	protected void handleTileEntities(Random rand)
	{
		for (BlockPos chestCoords : this.chests)
		{
			...
			if (chest != null && chest instanceof TileEntityMercuryTreasureChest)
			{
				ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);
				WeightedRandomChestContent.generateChestContents(rand, info.getItems(rand), (TileEntityMercuryTreasureChest) chest, info.getCount(rand));
	...
	{
		return new RoomTreasureMercury(dungeon, x, y, z, facing);
	}
}