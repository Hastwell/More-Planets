    ...
    protected void onImpact(MovingObjectPosition moving)
    {
        if (!this.worldObj.isRemote)
        {
            if (moving != null)
            {
                ...
                {
                    this.worldObj.setBlock(moving.blockX, moving.blockY + 1, moving.blockZ, PolongniusBlocks.fallen_polongnius_meteor, 0, 3);
                }
                if (moving.entityHit != null)
                {
                    moving.entityHit.attackEntityFrom(EntityPolongniusMeteor.causeMeteorDamage(this, this.shootingEntity), ConfigManagerCore.hardMode ? 12F : 6F);
                }
            }
