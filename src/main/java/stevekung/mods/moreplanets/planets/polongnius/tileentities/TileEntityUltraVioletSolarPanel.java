        ...
        for (int y = 1; y <= 2; y++)
        {
            for (int x = -1; x < 2; x++)
            {
                for (int z = -1; z < 2; z++)
                {
                    ...
                    {
                        FMLClientHandler.instance().getClient().effectRenderer.addBlockDestroyEffects(thisBlock.x + (y == 2 ? x : 0), thisBlock.y + y, thisBlock.z + (y == 2 ? z : 0), PolongniusBlocks.ultra_violet_solar_panel, Block.getIdFromBlock(PolongniusBlocks.ultra_violet_solar_panel) >> 12 & 255);
                    }
