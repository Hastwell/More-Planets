...
public class RoomBossPolongnius extends DungeonRoom
{
	public int sizeX;
	public int sizeY;
	public int sizeZ;
	...
	public RoomBossPolongnius(MapGenDungeon dungeon, int x, int y, int z, EnumFacing facing)
	{
		...
		for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
		{
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, Blocks.lava, 0);
					}
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, PolongniusBlocks.cheese_slime_egg, 0);
					}
					else
					{
	...
	{
		return new RoomBossPolongnius(dungeon, x, y, z, facing);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		this.worldObj.setBlockState(this.spawnerCoords, PolongniusBlocks.polongnius_block.getDefaultState(), 3);//TODO Boss Dungeon Spawner

		TileEntity tile = this.worldObj.getTileEntity(this.spawnerCoords);

		if (tile == null || !(tile instanceof TileEntityPolongniusDungeonSpawner))
		{
			TileEntityDungeonSpawner spawner = new TileEntityPolongniusDungeonSpawner();
