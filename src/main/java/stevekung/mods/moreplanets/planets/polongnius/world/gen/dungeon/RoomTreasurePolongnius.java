...
package stevekung.mods.moreplanets.planets.polongnius.world.gen.dungeon;
...
public class RoomTreasurePolongnius extends DungeonRoom
{
	int sizeX;
	int sizeY;
	int sizeZ;
	...
	public RoomTreasurePolongnius(MapGenDungeon dungeon, int x, int y, int z, EnumFacing facing)
	{
	...
	public void generate(Block[] chunk, byte[] meta, int cx, int cz)
	{
		...
		if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, PolongniusBlocks.polongnius_treasure_chest, 0))
		{
	...
	public ItemStack getGuaranteedLoot(Random rand)
	{
		...
		case 0:
		default:
			return new ItemStack(PolongniusBlocks.cheese_of_milk_cake, 1, 0);
		case 1:
			return new ItemStack(PolongniusItems.cheese_food, rand.nextInt(4) + 1, 0);
		}
	}
	...
	protected void handleTileEntities(Random rand)
	{
		for (BlockPos chestCoords : this.chests)
		{
			...
			if (chest != null && chest instanceof TileEntityPolongniusTreasureChest)
			{
				ChestGenHooks info = ChestGenHooks.getInfo(ChestGenHooks.DUNGEON_CHEST);
				WeightedRandomChestContent.generateChestContents(rand, info.getItems(rand), (TileEntityPolongniusTreasureChest) chest, info.getCount(rand));
				((TileEntityPolongniusTreasureChest) chest).setInventorySlotContents(rand.nextInt(((TileEntityPolongniusTreasureChest) chest).getSizeInventory()), this.getGuaranteedLoot(rand));
			}
		}
	}
	...
	{
		return new RoomTreasurePolongnius(dungeon, x, y, z, facing);
	}
}