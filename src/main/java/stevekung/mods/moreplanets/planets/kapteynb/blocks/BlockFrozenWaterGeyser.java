...
public class BlockFrozenWaterGeyser extends BlockBaseMP implements ITerraformableBlock, ITileEntityProvider
{
    ...
    public void randomDisplayTick(World world, BlockPos pos, IBlockState state, Random rand)
    {
        for (int i = 0; i < 5; i++)
        {
            ...
            {
                MorePlanetsCore.proxy.spawnParticle(EnumParticleTypesMP.FROZEN_WATER_GEYSER, pos.getX() + 0.5F, pos.getY() + 0.5F + rand.nextFloat(), pos.getZ() + 0.5F, 0.0D, 0.0D + rand.nextFloat(), 0.0D);
            }
        }
    }
