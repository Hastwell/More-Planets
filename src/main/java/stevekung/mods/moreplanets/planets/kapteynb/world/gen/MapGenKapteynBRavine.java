...
public class MapGenKapteynBRavine extends MapGenBase
{
                        ...
                        for (int l2 = i4; !flag2 && l2 < j2; ++l2)
                        {
                            for (int i3 = i2 + 1; !flag2 && i3 >= l3 - 1; --i3)
                            {
                                if (i3 >= 0 && i3 < 256)
                                {
                                    ...
                                    if (this.isOceanBlock(p_180707_5_, k2, i2, l2, p_180707_3_, p_180707_4_))
                                    {
                                        flag2 = true;
                                    }
    ...
    protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
        ...
        if (state.getBlock() == KapteynBBlocks.kapteyn_b_block || state.getBlock() == KapteynBBlocks.kapteyn_b_ice || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock())
        {
            if (y < 20)
            {
                data.setBlockState(x, y, z, KapteynBBlocks.frozen_water.getDefaultState());
            }
            else if (y < 10)
            {
