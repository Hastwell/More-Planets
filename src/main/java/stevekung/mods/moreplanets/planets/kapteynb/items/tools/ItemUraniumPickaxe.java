...
{
	public ItemUraniumPickaxe(String name, ToolMaterial par2EnumToolMaterial)
	{
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player, List list, boolean advanced)
	{
		...
		{
			list.add(EnumChatFormatting.GRAY + "Ice Crystal Tool Upgrade Required : 16");
		}
	}
	...
	public boolean onEntityItemUpdate(EntityItem item)
	{
		...
		if (item.worldObj.getBlock((int)Math.floor(item.posX), (int)Math.floor(item.posY), (int)Math.floor(item.posZ)) == KapteynBBlocks.frozen_water)
		{
			...
			{
				item.setEntityItemStack(new ItemStack(KapteynBToolsItems.ice_crystal_pickaxe));
