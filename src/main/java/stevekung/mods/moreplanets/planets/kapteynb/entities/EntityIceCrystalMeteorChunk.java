...
public class EntityIceCrystalMeteorChunk extends Entity implements IProjectile
{
    private int xTile = -1;
    private int yTile = -1;
    private int zTile = -1;
    ...
    public EntityIceCrystalMeteorChunk(World world)
    {
        super(world);
    ...
    public EntityIceCrystalMeteorChunk(World world, double x, double y, double z)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
        this.setSize(0.5F, 0.5F);
    ...
    public EntityIceCrystalMeteorChunk(World world, EntityLivingBase shooter, EntityLivingBase projectile, float velocity, float inaccuracy)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
    ...
    public EntityIceCrystalMeteorChunk(World world, EntityLivingBase shooter, float velocity)
    {
        super(world);
        this.renderDistanceWeight = 10.0D;
        this.shootingEntity = shooter;
                ...
                if (movingobjectposition.entityHit != null)
                {
                    ...
                    if (movingobjectposition.entityHit.attackEntityFrom(damagesource, k))
                    {
                        if (movingobjectposition.entityHit instanceof EntityLivingBase)
                        {
                            ...
                            this.worldObj.playSoundAtEntity(this, Block.soundTypeGlass.getBreakSound(), (Block.soundTypeGlass.getVolume() + 1.0F) / 2.0F, Block.soundTypeGlass.getFrequency() * 0.8F);
                            List<EntityLivingBase> living = this.worldObj.getEntitiesWithinAABB(EntityLivingBase.class, AxisAlignedBB.fromBounds(this.posX - 1.0D, this.posY - 1.0D, this.posZ - 1.0D, this.posX + 1.0D, this.posY + 1.0D, this.posZ + 1.0D));
        ...
        for (int i2 = 0; i2 < b0; ++i2)
        {
            for (int j1 = 0; j1 < b0; ++j1)
            {
                for (int k1 = 0; k1 < b0; ++k1)
                {
                    ...
                    this.worldObj.spawnParticle(EnumParticleTypes.BLOCK_CRACK, d00, d1, d2, d00 - this.posX - 0.5D, d1 - this.posY - 0.5D, d2 - this.posZ - 0.5D, new int[] { Block.getStateId(KapteynBBlocks.icy_poison_crystal.getDefaultState()) });
                }
            }
        }
    }
