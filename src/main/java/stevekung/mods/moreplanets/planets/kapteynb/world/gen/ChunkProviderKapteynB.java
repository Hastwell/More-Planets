...
{
	Block topBlockID = KapteynBBlocks.kapteyn_b_block;
	byte topBlockMeta = 0;
	Block fillBlockID = KapteynBBlocks.kapteyn_b_block;
	byte fillBlockMeta = 1;
	Block lowerBlockID = KapteynBBlocks.kapteyn_b_block;
	...
	public ChunkProviderKapteynB(World par1World, long par2, boolean par4)
	{
	...
	public void populate(IChunkProvider chunk, int chunkX, int chunkZ)
	{
		...
		for (int i = 0; doGen && i < 4; i++)
		{
			if (this.rand.nextInt(5) == 0)
			{
				...
				new WorldGenSplashBlock(KapteynBBlocks.frozen_water_geyser, 0, KapteynBBlocks.kapteyn_b_block, 0).generate(this.worldObj, this.rand, x, y, z);
			}
		}
