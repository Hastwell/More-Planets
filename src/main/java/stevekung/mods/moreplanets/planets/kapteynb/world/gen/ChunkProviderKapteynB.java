...
public class ChunkProviderKapteynB extends ChunkProviderBaseMP
{
	...
	public void populate(IChunkProvider chunk, int chunkX, int chunkZ)
	{
		...
		this.biomeDecorator.decorate(this.worldObj, this.rand, BiomeGenBaseKapteynB.kapteynB, pos);
		...
		for (int i = 0; i < 4; i++)
		{
			if (this.rand.nextInt(5) == 0)
			{
				...
				new WorldGenSplashBlock(KapteynBBlocks.frozen_water_geyser.getDefaultState(), KapteynBBlocks.kapteyn_b_block.getDefaultState()).generate(this.worldObj, this.rand, new BlockPos(x1, y1, z1));
			}
		}
