...
public class MapGenNibiruRavine extends MapGenBase
{
    ...
    protected void digBlock(ChunkPrimer data, int x, int y, int z, int chunkX, int chunkZ, boolean foundTop)
    {
        ...
        if (state.getBlock() == NibiruBlocks.nibiru_block || state.getBlock() == NibiruBlocks.infected_grass || state.getBlock() == NibiruBlocks.infected_dirt || state.getBlock() == NibiruBlocks.helium_block || state.getBlock() == top.getBlock() || state.getBlock() == filler.getBlock())
        {
            if (y < 20)
            {
