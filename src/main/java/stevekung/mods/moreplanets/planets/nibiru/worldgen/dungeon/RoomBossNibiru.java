...
public class RoomBossNibiru extends DungeonRoom
{
    public int sizeX;
    public int sizeY;
    public int sizeZ;
    ...
    public RoomBossNibiru(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
        ...
        for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
        {
            for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
            {
                for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
                {
                    ...
                    {
                        this.placeBlock(chunk, meta, i, j, k, cx, cz, NibiruBlocks.infected_zombie_egg, 0);
                    }
                    else
                    {
    ...
    protected void handleTileEntities(Random rand)
    {
        ...
        this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, NibiruBlocks.nibiru_block, 13, 3);
        ...
        if (tile == null || !(tile instanceof TileEntityNibiruDungeonSpawner))
        {
            final TileEntityDungeonSpawner spawner = new TileEntityNibiruDungeonSpawner();
    ...
    {
        return new RoomBossNibiru(dungeon, x, y, z, dir);
    }
}