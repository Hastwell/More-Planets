...
{
    Block topBlockID = DionaBlocks.diona_block;
    byte topBlockMeta = 0;
    Block fillBlockID = DionaBlocks.diona_block;
    byte fillBlockMeta = 1;
    Block lowerBlockID = DionaBlocks.diona_block;
    ...
    private BiomeDecoratorDiona dionaBiomeDecorator = new BiomeDecoratorDiona();
    private World worldObj;
    private Random rand;
    private MapGenDungeon dungeonGenerator = new MapGenDungeon(DionaBlocks.diona_block, 14, 8, 16, 4);
    {
        ...
        this.dungeonGenerator.otherRooms.add(new RoomSpawnerMP(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new RoomSpawnerMP(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new RoomChestsDiona(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.otherRooms.add(new RoomChestsDiona(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.bossRooms.add(new RoomBossDiona(null, 0, 0, 0, ForgeDirection.UNKNOWN));
        this.dungeonGenerator.treasureRooms.add(new RoomTreasureDiona(null, 0, 0, 0, ForgeDirection.UNKNOWN));
    }
    ...
    public ChunkProviderDiona(World par1World, long par2, boolean par4)
    {
    ...
    public void generateTerrain(int chunkX, int chunkZ, Block[] idArray, byte[] metaArray)
    {
        ...
        for (int x = 0; x < ChunkProviderDiona.CHUNK_SIZE_X; x++)
        {
            for (int z = 0; z < ChunkProviderDiona.CHUNK_SIZE_Z; z++)
            {
                ...
                for (int y = 0; y < ChunkProviderDiona.CHUNK_SIZE_Y; y++)
                {
                    if (y < ChunkProviderDiona.MID_HEIGHT + yDev)
                    {
        ...
        for (int cx = chunkX - 2; cx <= chunkX + 2; cx++)
        {
            for (int cz = chunkZ - 2; cz <= chunkZ + 2; cz++)
            {
                for (int x = 0; x < ChunkProviderDiona.CHUNK_SIZE_X; x++)
                {
                    for (int z = 0; z < ChunkProviderDiona.CHUNK_SIZE_Z; z++)
                    {
                        if (Math.abs(this.randFromPoint(cx * 16 + x, (cz * 16 + z) * 1000)) < this.noiseGen4.getNoise(x * ChunkProviderDiona.CHUNK_SIZE_X + x, cz * ChunkProviderDiona.CHUNK_SIZE_Z + z) / ChunkProviderDiona.CRATER_PROB)
                        {
    ...
    {
        this.dionaBiomeDecorator.decorate(par1World, par2Random, par3, par4);
    }
