...
{
    private ModelBase rocketNoFlagModel;

    public ItemRendererTier4Rocket(ModelBase rocketNoFlagModel)
    {
        super(new EntityTier4Rocket(FMLClientHandler.instance().getClient().theWorld), new ModelRocketMP(), new ResourceLocation("diona:textures/model/tier4_rocket.png"));
        this.rocketNoFlagModel = rocketNoFlagModel;
    }
    ...
    protected void renderSpaceship(ItemRenderType type, RenderBlocks render, ItemStack item, float translateX, float translateY, float translateZ)
    {
        ...
        {
            FMLClientHandler.instance().getClient().renderEngine.bindTexture(this.texture);
            this.rocketNoFlagModel.render(this.spaceship, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
            GL11.glPopMatrix();
        }
    ...
    public void transform(ItemStack itemstack, ItemRenderType type)
    {
        ...
        if (type == ItemRenderType.EQUIPPED)
        {
            ...
            if (player != null && player.ridingEntity != null && player.ridingEntity instanceof EntityTier4Rocket)
            {
