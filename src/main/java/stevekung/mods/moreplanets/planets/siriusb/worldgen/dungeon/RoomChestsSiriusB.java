...
package stevekung.mods.moreplanets.planets.siriusb.worldgen.dungeon;
...
public class RoomChestsSiriusB extends DungeonRoom
{
    int sizeX;
    int sizeY;
    int sizeZ;
    ...
    public RoomChestsSiriusB(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
    ...
    public void generate(Block[] chunk, byte[] meta, int cx, int cz)
    {
        ...
        if (this.placeBlock(chunk, meta, hx, this.posY, hz, cx, cz, SiriusBBlocks.sirius_b_ancient_chest, 0))
        {
    ...
    {
        return new RoomChestsSiriusB(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        ...
        {
            this.worldObj.setBlock(this.chests.get(0).posX, this.chests.get(0).posY, this.chests.get(0).posZ, SiriusBBlocks.sirius_b_ancient_chest, 0, 2);
            final TileEntitySiriusBAncientChest chest = (TileEntitySiriusBAncientChest) this.worldObj.getTileEntity(this.chests.get(0).posX, this.chests.get(0).posY, this.chests.get(0).posZ);
