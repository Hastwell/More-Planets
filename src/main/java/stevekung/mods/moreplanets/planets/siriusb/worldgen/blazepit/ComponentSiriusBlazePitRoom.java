...
package stevekung.mods.moreplanets.planets.siriusb.worldgen.blazepit;
        ...
        for (int y = 0; y < this.height; y++)
        {
            for (int x = 0; x < 7; x++)
            {
                for (int z = 0; z < 7; z++)
                {
                    ...
                    {
                        this.placeBlockAtCurrentPosition(world, SiriusBBlocks.sirius_b_block, 9, x, y, z, this.getBoundingBox());
                    }
                    else
                    {
                ...
                for (int z = 0; z < 1; z++)
                {
                    for (int i = -2; i < 7; i++)
                    {
                        for (int j = -2; j < 7; j++)
                        {
                            ...
                            {
                                this.placeBlockAtCurrentPosition(world, SiriusBBlocks.sirius_b_block, 9, x + i, y, z + j, this.getBoundingBox());
                            }
                        }
                    }
                }
                    ...
                    if (y % 4 == 0 && rand.nextInt(20) == 0)
                    {
                        for (int i = -2; i < 2; i++)
                        {
                            for (int j = -2; j < 2; j++)
                            {
                                ...
                                if (rand.nextInt(5) == 0 && world.getBlock(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y + 1, this.getBoundingBox().minZ + z + j) == Blocks.air && world.getBlock(this.getBoundingBox().minX + x + i, this.getBoundingBox().minY + y, this.getBoundingBox().minZ + z + j) == SiriusBBlocks.sirius_b_block)
                                {
                                    this.placeBlockAtCurrentPosition(world, SiriusBBlocks.sirius_blaze_egg, 0, x + i, y + 1, z + j, this.getBoundingBox());
                                }
                            }
                        }
                        ...
                        if (rand.nextInt(10) == 0)
                        {
                            if (x > 0 && x < 7 && z > 0 && z < 7)
                            {
                                ...
                                {
                                    this.placeBlockAtCurrentPosition(world, SiriusBBlocks.sirius_blaze_egg, 0, x, y + 2, z, this.getBoundingBox());
                                }
                            }
                        }
                    }
