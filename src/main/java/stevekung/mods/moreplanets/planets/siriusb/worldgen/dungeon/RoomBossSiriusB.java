...
public class RoomBossSiriusB extends DungeonRoom
{
	...
	public RoomBossSiriusB(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
	{
		...
		for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
		{
			for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
			{
				for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
				{
					...
					{
						this.placeBlock(chunk, meta, i, j, k, cx, cz, SiriusBBlocks.sirius_blaze_egg, 0);
					}
					else
					{
	...
	{
		return new RoomBossSiriusB(dungeon, x, y, z, dir);
	}
	...
	protected void handleTileEntities(Random rand)
	{
		...
		this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, SiriusBBlocks.sirius_b_block, 10, 3);
		...
		{
			final TileEntityDungeonSpawner spawner = new TileEntitySiriusBDungeonSpawner();
