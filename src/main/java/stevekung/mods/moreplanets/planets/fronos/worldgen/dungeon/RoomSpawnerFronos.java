...
public class RoomSpawnerFronos extends DungeonRoom
{
    ...
    public RoomSpawnerFronos(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
            ...
            for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
            {
                for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
                {
                    ...
                    else
                    {
                        ...
                        {
                            this.placeBlock(chunk, meta, i, j, k, cx, cz, FronosBlocks.cheese_web, 0);
                        }
                    }
                }
            }
    ...
    {
        return new RoomSpawnerFronos(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        for (final ChunkCoordinates spawnerCoords : this.spawners)
        {
            if (this.worldObj.getBlock(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ) == Blocks.mob_spawner)
            {
                ...
                if (spawner != null)
                {
                    spawner.func_145881_a().setEntityName(RoomSpawnerFronos.getMob(rand));
                }
            }
        }
    }
