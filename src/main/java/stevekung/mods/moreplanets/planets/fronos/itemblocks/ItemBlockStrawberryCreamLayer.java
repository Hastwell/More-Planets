...
public class ItemBlockStrawberryCreamLayer extends ItemBlockMorePlanets
{
	public ItemBlockStrawberryCreamLayer(Block block)
	{
		super(block);
	}
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.strawberry_cream_layer || (Integer)state.getValue(BlockSnowLayerMP.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}