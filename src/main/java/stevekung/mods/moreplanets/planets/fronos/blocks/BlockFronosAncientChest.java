...
public class BlockFronosAncientChest extends BlockAncientChestMP
{
    ...
    public boolean onBlockActivated(World par1World, int par2, int par3, int par4, EntityPlayer par5EntityPlayer, int par6, float par7, float par8, float par9)
    {
        ...
        else if (par1World.rand.nextInt(10) == 0 && !par5EntityPlayer.capabilities.isCreativeMode)
        {
            if (!par1World.isRemote)
            {
                EntityJellySlime jelly = new EntityJellySlime(par1World);
                jelly.setPosition(par2 + 0.5, par3 + 2, par4 + 0.5);
                jelly.setTameSkin(par1World.rand.nextInt(7));
                par1World.spawnEntityInWorld(jelly);
            }
        }
        else
        {
    ...
    public IInventory getInventory(World par1World, int par2, int par3, int par4)
    {
        ...
        else
        {
            ...
            {
                object = new InventoryLargeChest(StatCollector.translateToLocal("container.fronos.ancientchest.name"), (TileEntityFronosAncientChest)par1World.getTileEntity(par2 - 1, par3, par4), (IInventory)object);
            }
            if (par1World.getBlock(par2 + 1, par3, par4) == this)
            {
                object = new InventoryLargeChest(StatCollector.translateToLocal("container.fronos.ancientchest.name"), (IInventory)object, (TileEntityFronosAncientChest)par1World.getTileEntity(par2 + 1, par3, par4));
            }
            if (par1World.getBlock(par2, par3, par4 - 1) == this)
            {
                object = new InventoryLargeChest(StatCollector.translateToLocal("container.fronos.ancientchest.name"), (TileEntityFronosAncientChest)par1World.getTileEntity(par2, par3, par4 - 1), (IInventory)object);
            }
            if (par1World.getBlock(par2, par3, par4 + 1) == this)
            {
                object = new InventoryLargeChest(StatCollector.translateToLocal("container.fronos.ancientchest.name"), (IInventory)object, (TileEntityFronosAncientChest)par1World.getTileEntity(par2, par3, par4 + 1));
            }
    ...
    {
        return "fronos:fronos_ancient_chest";
    }
