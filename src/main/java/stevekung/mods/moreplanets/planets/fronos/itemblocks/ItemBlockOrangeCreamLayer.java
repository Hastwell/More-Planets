...
public class ItemBlockOrangeCreamLayer extends ItemBlockMorePlanets
{
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.orange_cream_layer || (Integer)state.getValue(BlockSnowLayerMP.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}