...
package stevekung.mods.moreplanets.planets.fronos.blocks.fluids;
...
public class BlockFluidOvaltine extends BlockFluidBaseMP
{
    public BlockFluidOvaltine(String name)
    {
    ...
    public void randomDisplayTick(World world, BlockPos pos, IBlockState state, Random rand)
    {
        if (rand.nextInt(1) == 0)
        {
            MorePlanetsCore.proxy.spawnParticle(EnumParticleTypesMP.OVALTINE, pos.getX() + rand.nextFloat(), pos.getY() + 1, pos.getZ() + rand.nextFloat());
        }
        if (rand.nextInt(10) == 0 && World.doesBlockHaveSolidTopSurface(world, pos.down()) && !world.getBlockState(pos.down(2)).getBlock().getMaterial().blocksMovement())
        {
            ...
            MorePlanetsCore.proxy.spawnParticle(EnumParticleTypesMP.OVALTINE_DRIP, d5, d6, d7);
        }
    }
