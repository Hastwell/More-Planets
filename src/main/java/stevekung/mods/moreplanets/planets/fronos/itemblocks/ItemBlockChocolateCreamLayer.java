...
public class ItemBlockChocolateCreamLayer extends ItemBlockMorePlanets
{
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.chocolate_cream_layer || (Integer)state.getValue(BlockSnowLayerMP.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}