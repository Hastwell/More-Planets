...
public class EntityCreamSlime extends EntityLiving implements IMob
{
    ...
    public EntityCreamSlime(World par1World)
    {
        super(par1World);
        ...
        this.setSlimeSize(i);
        this.setTameSkin(this.rand.nextInt(6));
    }
    ...
    public void writeEntityToNBT(NBTTagCompound par1NBTTagCompound)
    {
        ...
        par1NBTTagCompound.setInteger("SlimeType", this.getCreamSlimeType());
    ...
    public void onUpdate()
    {
        ...
        if (this.onGround && !flag)
        {
            ...
            for (int j = 0; j < i * 8; ++j)
            {
                ...
                if (particles == 0)
                {
                    MorePlanetsCore.proxy.spawnParticle("vanillaBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
                else if (particles == 1)
                {
                    MorePlanetsCore.proxy.spawnParticle("chocolateBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
                else if (particles == 2)
                {
                    MorePlanetsCore.proxy.spawnParticle("strawberryBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
                else if (particles == 3)
                {
                    MorePlanetsCore.proxy.spawnParticle("orangeBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
                else if (particles == 4)
                {
                    MorePlanetsCore.proxy.spawnParticle("teaBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
                else if (particles == 5)
                {
                    MorePlanetsCore.proxy.spawnParticle("lemonBall", this.posX + f2, this.boundingBox.minY, this.posZ + f3);
                }
            }
    ...
    public void setDead()
    {
        ...
        if (!this.worldObj.isRemote && i > 1 && this.getHealth() <= 0.0F)
        {
            ...
            for (int k = 0; k < j; ++k)
            {
                ...
                entityslime.setTameSkin(this.getCreamSlimeType());
