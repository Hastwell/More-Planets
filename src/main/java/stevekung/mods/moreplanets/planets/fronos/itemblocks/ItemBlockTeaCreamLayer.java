...
public class ItemBlockTeaCreamLayer extends ItemBlockMorePlanets
{
	public ItemBlockTeaCreamLayer(Block block)
	{
		super(block);
	}
	...
	{
		IBlockState state = world.getBlockState(pos);
		return state.getBlock() != FronosBlocks.tea_cream_layer || (Integer)state.getValue(BlockSnowLayerMP.LAYERS) > 7 ? super.canPlaceBlockOnSide(world, pos, side, player, stack) : true;
	}
}