...
public class WorldChunkManagerFronos extends WorldChunkManagerSpace
{
    ...
    @Override
    public BiomeGenBase[] getBiomesForGeneration(BiomeGenBase[] biome, int x, int z, int length, int width)
    {
        int[] arrayOfInts = this.unzoomedBiomes.getInts(x, z, length, width);

        if (biome == null || biome.length < length * width)
        {
            biome = new BiomeGenBase[length * width];
        }
        for (int i = 0; i < length * width; i++)
        {
            if (arrayOfInts[i] >= 0)
            {
                biome[i] = BiomeGenBase.getBiome(arrayOfInts[i]);
            }
            else
            {
                biome[i] = BiomeGenBaseFronos.coconutForest;
            }
        }
        return biome;
    }
    ...
    @Override
    public BiomeGenBase[] loadBlockGeneratorData(BiomeGenBase[] biome, int x, int z, int width, int depth)
    {
        return this.getBiomeGenAt(biome, x, z, width, depth, true);
    }
    ...
    public BiomeGenBase[] getBiomeGenAt(BiomeGenBase[] biome, int x, int z, int width, int length, boolean cacheFlag)
    {
        ...
        {
            biome = new BiomeGenBase[width * length];
        }
        if (cacheFlag && width == 16 && length == 16 && (x & 0xF) == 0 && (z & 0xF) == 0)
        {
            BiomeGenBase[] abiomegenbase = this.myBiomeCache.getCachedBiomes(x, z);
            System.arraycopy(abiomegenbase, 0, biome, 0, width * length);
            return biome;
        }
        for (int i = 0; i < width * length; i++)
        {
            if (ai[i] >= 0)
            {
                biome[i] = BiomeGenBase.getBiome(ai[i]);
            }
            else
            {
                biome[i] = BiomeGenBaseFronos.coconutForest;
    ...
    @Override
    public boolean areBiomesViable(int x, int z, int range, List list)
    {
        int i = x - range >> 2;
        int j = z - range >> 2;
        int k = x + range >> 2;
    ...
    @Override
    public BlockPos findBiomePosition(int x, int z, int range, List list, Random rand)
    {
        int i = x - range >> 2;
        int j = z - range >> 2;
        int k = x + range >> 2;
        ...
        for (int l1 = 0; l1 < ai.length; l1++)
        {
            ...
            if (list.contains(biomegenbase) && (chunkposition == null || rand.nextInt(k1 + 1) == 0))
            {
