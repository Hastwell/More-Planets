...
public class BlockFronosLeaves extends BlockLeavesBase implements IShearable
{
    ...
    private FronosLeafCategory category;
    int[] adjacentTreeBlocks;

    public BlockFronosLeaves(String name, FronosLeafCategory cat)
    {
    ...
    {
        this.textures = new IIcon[3][BlockFronosLeaves.leaves.length];

        for (int i = 0; i < BlockFronosLeaves.leaves.length; ++i)
        {
            this.textures[0][i] = iconRegister.registerIcon("fronos:leaves_" + BlockFronosLeaves.leaves[i] + "_fancy");
            this.textures[1][i] = iconRegister.registerIcon("fronos:leaves_" + BlockFronosLeaves.leaves[i] + "_fast");
        }
    }
    ...
    public IIcon getIcon(int side, int meta)
    {
        ...
        return this.textures[!this.isOpaqueCube() ? 0 : 1][type >= BlockFronosLeaves.leaves.length ? 0 : type];
    }
    ...
    {
        int type = meta & 3 + this.category.ordinal() * 4;
        return BlockFronosLeaves.leaves[type >= BlockFronosLeaves.leaves.length ? 0 : type];
    }
