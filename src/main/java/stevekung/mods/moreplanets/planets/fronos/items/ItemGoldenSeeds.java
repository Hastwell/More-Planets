...
public class ItemGoldenSeeds extends ItemMorePlanet implements IPlantable
{
    public ItemGoldenSeeds(String name)
    {
        super();
        this.setUnlocalizedName(name);
        this.setTextureName("fronos:golden_seeds");
    }
    ...
    public boolean onItemUse(ItemStack itemStack, EntityPlayer player, World world, int par4, int par5, int par6, int par7, float par8, float par9, float par10)
    {
        ...
        else if (player.canPlayerEdit(par4, par5, par6, par7, itemStack) && player.canPlayerEdit(par4, par5 + 1, par6, par7, itemStack))
        {
            ...
            {
                world.setBlock(par4, par5 + 1, par6, FronosBlocks.golden_crops);
                --itemStack.stackSize;
                return true;
            }
            else
