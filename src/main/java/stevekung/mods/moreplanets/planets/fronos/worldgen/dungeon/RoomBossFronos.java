...
public class RoomBossFronos extends DungeonRoom
{
    ...
    public RoomBossFronos(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
        ...
        for (int i = this.posX - 1; i <= this.posX + this.sizeX; i++)
        {
            for (int k = this.posZ - 1; k <= this.posZ + this.sizeZ; k++)
            {
                for (int j = this.posY - 1; j <= this.posY + this.sizeY; j++)
                {
                    ...
                    {
                        this.placeBlock(chunk, meta, i, j, k, cx, cz, FronosBlocks.jelly_slime_egg, this.rand.nextInt(8));
                    }
                    else
                    {
    ...
    {
        return new RoomBossFronos(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        ...
        this.worldObj.setBlock(this.spawnerCoords.posX, this.spawnerCoords.posY, this.spawnerCoords.posZ, FronosBlocks.fronos_block, 15, 3);
        ...
        {
            final TileEntityDungeonSpawner spawner = new TileEntityFronosDungeonSpawner();
