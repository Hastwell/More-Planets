...
public class ConfigManagerMP
{
	...
	private void setDefaultValues()
	{
		try
		{
			...
			ConfigManagerMP.disableInfectedGas = this.configuration.get(Configuration.CATEGORY_GENERAL, "Disable Infected Gas on Nibiru", false, "If you disable this option. When you mining any blocks on the Nibiru you will getting Infected Gas. and Infected Gas does not affected when equipped Tier 2 Thermal Armor. Default is false.").getBoolean(false);
		...
		{
			MPLog.error("Found a problem when loading configuration");
			e.printStackTrace();
		}
		finally
		{
