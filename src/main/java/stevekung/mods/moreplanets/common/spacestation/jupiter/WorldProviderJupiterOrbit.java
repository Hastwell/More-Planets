...
import stevekung.mods.moreplanets.common.spacestation.OrbitSpinSaveDataMP;
import stevekung.mods.moreplanets.core.MorePlanetsCore;

public class WorldProviderJupiterOrbit extends WorldProviderOrbit
{
	public int spaceStationDimensionID;

	private static float GFORCE = 9.81F / 400F; //gravity in metres per tick squared

	private OrbitSpinSaveDataMP savefile;
	...
	public void updateWeather()
	{
		...
		if (!this.worldObj.isRemote)
		{
			...
			{
				this.savefile = OrbitSpinSaveDataMP.initWorldData(this.worldObj);
			...
			if (this.doSpinning)
			{
				...
				for (Entity e : this.loadedEntities)
				{
					if ((e instanceof EntityItem || e instanceof EntityLivingBase && !(e instanceof EntityPlayer) || e instanceof EntityTNTPrimed || e instanceof EntityFallingBlock) && !e.onGround)
					{
						...
						if (e.getEntityBoundingBox().maxX >= this.ssBoundsMinX && e.getEntityBoundingBox().minX <= this.ssBoundsMaxX && e.getEntityBoundingBox().maxY >= this.ssBoundsMinY && e.getEntityBoundingBox().minY <= this.ssBoundsMaxY && e.getEntityBoundingBox().maxZ >= this.ssBoundsMinZ && e.getEntityBoundingBox().minZ <= this.ssBoundsMaxZ)
						{
							...
							BLOCKCHECK:
								for (int x = MathHelper.floor_double(e.getEntityBoundingBox().minX - 0.2D); x <= xmx; x++)
								{
									for (int y = ym; y <= yy; y++)
									{
										for (int z = zm; z <= zz; z++)
										{
											if (this.worldObj.isBlockLoaded(new BlockPos(x, y, z)) && Blocks.air != this.worldObj.getBlockState(new BlockPos(x, y, z)).getBlock())
											{
												freefall = false;
												break BLOCKCHECK;
											}
	...
	public void postVanillaMotion(EntityPlayerSP p)
	{
		...
		if (freefall)
		{
			//p.getYOffset() = 0D;  //Undo the sneak height adjust TODO
		}
	...
	{
		if (this.oneSSBlock == null || this.oneSSBlock.getBlockID(this.worldObj).isAir(this.worldObj, new BlockPos(this.oneSSBlock.x, this.oneSSBlock.y, this.oneSSBlock.z)))
		{
			if (baseBlock != null)
			{
			...
			for (BlockVec3 vec : currentLayer)
			{
				...
				for (int side = 0; side < 6; side++)
				{
					...
					if (!this.checked.contains(sideVec))
					{
						...
						if (!b.isAir(this.worldObj, new BlockPos(sideVec.x, sideVec.y, sideVec.z)) && b != null)
						{
							nextLayer.add(sideVec);

							if (bStart.isAir(this.worldObj, new BlockPos(this.oneSSBlock.x, this.oneSSBlock.y, this.oneSSBlock.z)))
							{
							...
							if (!(b instanceof BlockLiquid))
							{
								...
								m = b.getBlockHardness(this.worldObj, new BlockPos(sideVec.x, sideVec.y, sideVec.z));
	...
	public void updateSpinSpeed()
	{
		if (this.momentOfInertia > 0F)
		{
			...
			else
			{
				...
				this.angularVelocityTarget = MathHelper.sqrt_float(WorldProviderJupiterOrbit.GFORCE / maxR) / 2;
