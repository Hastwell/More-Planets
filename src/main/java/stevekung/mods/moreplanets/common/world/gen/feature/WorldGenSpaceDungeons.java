...
public class WorldGenSpaceDungeons extends WorldGenerator
{
	...
	public boolean generate(World world, Random rand, BlockPos pos)
	{
		...
		if (k1 >= 1 && k1 <= 5)
		{
			for (l1 = j; l1 <= k; ++l1)
			{
				for (i2 = 3; i2 >= -1; --i2)
				{
					for (j2 = i1; j2 <= j1; ++j2)
					{
						...
						{
							if (world.getBlockState(blockpos1).getBlock() != this.block[0])
							{
						...
						else if (world.getBlockState(blockpos1).getBlock().getMaterial().isSolid() && world.getBlockState(blockpos1).getBlock() != this.block[0])
						{
							if (i2 == -1 && rand.nextInt(4) != 0)
							{
								world.setBlockState(blockpos1, this.block[2].getStateFromMeta(this.mossMeta), 2);
							}
							else
							{
								world.setBlockState(blockpos1, this.block[1].getStateFromMeta(this.cobbleMeta), 2);
							}
						}
					}
				}
				...
				while (true)
				{
					if (i2 < 3)
					{
						...
						if (world.isAirBlock(blockpos2))
						{
							...
							if (k2 == 1)
							{
								world.setBlockState(blockpos2, ((BlockAncientChestMP)this.block[0]).correctFacing(world, blockpos2, this.block[0].getDefaultState()), 2);
			...
			/*for (Method met : MobSpawnerBaseLogic.class.getDeclaredMethods()) TODO
			{
				met.setAccessible(true);

				if (met.getName().equals("getEntityNameToSpawn"))
				{

				}
			}*/
			MPLog.debug("Generate spawner at : %s %s %s", pos.getX(), pos.getY(), pos.getZ());
			return true;
		}
		else
		{
