...
public class WorldGenLiquidLakes extends WorldGenerator
{
    ...
    public WorldGenLiquidLakes(Block block, Block stone, int meta)
    {
        this.liquid = block;
        this.stone = stone;
        this.meta = meta;
                ...
                for (k1 = 0; k1 < 16; ++k1)
                {
                    for (k = 0; k < 8; ++k)
                    {
                        ...
                        if (flag)
                        {
                            ...
                            if (k < 4 && !material.isSolid() && world.getBlockState(pos.add(j, k, k1)).getBlock() != this.liquid)
                            {
                                return false;
                            }
                        }
            ...
            for (j = 0; j < 16; ++j)
            {
                for (k1 = 0; k1 < 16; ++k1)
                {
                    for (k = 0; k < 8; ++k)
                    {
                        ...
                        {
                            world.setBlockState(pos.add(j, k, k1), k >= 4 ? Blocks.air.getDefaultState() : this.liquid.getDefaultState(), 2);
                        }
                    }
                }
            }
                ...
                for (j = 0; j < 16; ++j)
                {
                    for (k1 = 0; k1 < 16; ++k1)
                    {
                        for (k = 0; k < 8; ++k)
                        {
                            ...
                            {
                                world.setBlockState(pos.add(j, k, k1), this.stone.getStateFromMeta(this.meta), 2);
                            }
                        }
                    }
                }
