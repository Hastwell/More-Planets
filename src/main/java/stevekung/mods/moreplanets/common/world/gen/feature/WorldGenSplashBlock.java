...
public class WorldGenSplashBlock extends WorldGenerator
{
    ...
    public boolean generate(World world, Random rand, BlockPos pos)
    {
        for (int i = 0; i < 64; i++)
        {
            BlockPos posnew = pos.add(rand.nextInt(8) - rand.nextInt(8), rand.nextInt(4) - rand.nextInt(4), rand.nextInt(8) - rand.nextInt(8));

            if (world.getBlockState(posnew.down()) == this.blockToGen.getStateFromMeta(this.metaToGen))
            {
                world.setBlockState(posnew.down(), this.block.getStateFromMeta(this.meta), 2);
            }
        }
        return true;
    }
