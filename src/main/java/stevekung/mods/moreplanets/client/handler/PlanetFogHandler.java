...
{
    private double fogX, fogZ;
    private boolean fogInit;
    private float fogFarPlaneDistance;
    ...
    public void onRenderFog(EntityViewRenderEvent.RenderFogEvent event)
    {
        ...
        if (playerX == this.fogX && playerZ == this.fogZ && this.fogInit)
        {
            this.renderFog(event.fogMode, this.fogFarPlaneDistance, 0.75f);
            return;
        }
        ...
        for (int x = -distance; x <= distance; ++x)
        {
            ...
            {
                BiomeGenBase biome = world.getBiomeGenForCoords(new BlockPos(playerX + x, playerY, playerZ + z));
        ...
        this.fogX = entity.posX;
        this.fogZ = entity.posZ;
        this.fogFarPlaneDistance = Math.min(farPlaneDistance, event.farPlaneDistance);
        this.renderFog(event.fogMode, this.fogFarPlaneDistance, farPlaneDistanceScale);
    }
    ...
    private static Vec3 getFogBlendColour(World world, EntityLivingBase playerEntity, int playerX, int playerY, int playerZ, float defR, float defG, float defB, double renderPartialTicks)
    {
        ...
        {
            return new Vec3(defR, defG, defB);
        }
