...
public class SkyProviderMarsOrbit extends IRenderHandler
{
    ...
    public void render(float partialTicks, WorldClient world, Minecraft mc)
    {
        ...
        var23.getWorldRenderer().startDrawingQuads();
        var23.getWorldRenderer().addVertexWithUV(-size, -100.0D, size, 0.0D, 0.0D);
        var23.getWorldRenderer().addVertexWithUV(size, -100.0D, size, 1.0D, 0.0D);
        var23.getWorldRenderer().addVertexWithUV(size, -100.0D, -size, 1.0D, 1.0D);
        var23.getWorldRenderer().addVertexWithUV(-size, -100.0D, -size, 0.0D, 1.0D);
        var23.draw();
