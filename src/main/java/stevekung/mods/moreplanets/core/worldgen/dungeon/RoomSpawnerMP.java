...
package stevekung.mods.moreplanets.core.worldgen.dungeon;
...
public class RoomSpawnerMP extends DungeonRoom
{
    ...
    public RoomSpawnerMP(MapGenDungeon dungeon, int posX, int posY, int posZ, ForgeDirection entranceDir)
    {
    ...
    {
        return new RoomSpawnerMP(dungeon, x, y, z, dir);
    }
    ...
    protected void handleTileEntities(Random rand)
    {
        for (ChunkCoordinates spawnerCoords : this.spawners)
        {
            ...
            {
                TileEntityMobSpawner spawner = (TileEntityMobSpawner) this.worldObj.getTileEntity(spawnerCoords.posX, spawnerCoords.posY, spawnerCoords.posZ);
                if (spawner != null)
                {
                    spawner.func_145881_a().setEntityName(RoomSpawnerMP.getMob(rand));
                }
            }
        }
    }
