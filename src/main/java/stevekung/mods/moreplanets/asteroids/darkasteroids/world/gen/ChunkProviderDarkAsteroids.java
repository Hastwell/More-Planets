...
public class ChunkProviderDarkAsteroids extends ChunkProviderGenerate
{
    ...
    Block DIRT = DarkAsteroidBlocks.alien_dirt;
    byte DIRT_META = 0;
    Block GRASS = DarkAsteroidBlocks.alien_grass;
    byte GRASS_META = 0;
    Block LIGHT = DarkAsteroidBlocks.alien_glowstone;
    byte LIGHT_META = 0;

    Block LAVA = IoBlocks.black_io_lava;
    byte LAVA_META = 0;
    ...
    public Chunk provideChunk(int chunkX, int chunkZ)
    {
        ...
        this.generateTerrain(chunkX, chunkZ, chunkPrimer, false);
        ...
        if (!this.largeAsteroids.isEmpty())
        {
            for (AsteroidData asteroidIndex : this.largeAsteroids)
            {
                ...
                if (this.rand.nextInt(ChunkProviderDarkAsteroids.TREE_CHANCE) == 0)
                {
                    ...
                    new WorldGenTreeMP(DarkAsteroidBlocks.alien_log, DarkAsteroidBlocks.alien_leaves, 0, 0, DarkAsteroidBlocks.alien_sapling, null, DarkAsteroidBlocks.alien_grass, DarkAsteroidBlocks.alien_dirt, null).generate(this.worldObj, this.rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
                }
                ...
                if (this.rand.nextInt(ChunkProviderDarkAsteroids.LAVA_CHANCE) == 0)
                {
                    int i = this.rand.nextInt(16) + x + 8;
                    int k = this.rand.nextInt(16) + z + 8;
                    new WorldGenLiquidLakes(this.LAVA, IoBlocks.io_block, 5).generate(this.worldObj, this.rand, new BlockPos(i, this.getTerrainHeightAt(i - x, k - z, sizeYArray, xMin, zMin, zSize, asteroidY, asteroidSize), k));
                }
