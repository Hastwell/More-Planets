...
public class BlockAlienSapling extends BlockSapling
{
    ...
    {
        this.textures = new IIcon[BlockAlienSapling.saplings.length];

        for (int i = 0; i < BlockAlienSapling.saplings.length; ++i)
        {
            this.textures[i] = iconRegister.registerIcon("mpcore:darkasteroids/sapling_" + BlockAlienSapling.saplings[i]);
        }
    }
    ...
    {
        if (meta < 0 || meta >= BlockAlienSapling.saplings.length)
        {
            meta = 0;
        }
    ...
    public boolean isValidPosition(World world, int x, int y, int z, int metadata)
    {
        ...
        {
        default:
            return block == Blocks.grass || block == Blocks.dirt || block == DarkAsteroidsBlocks.alien_dirt || block.canSustainPlant(world, x, y - 1, z, ForgeDirection.UP, this);
        }
    }
