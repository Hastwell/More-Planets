    ...
    public Vector3 getPlayerSpawnLocation(WorldServer world, EntityPlayerMP player)
    {
        if (player != null)
        {
            ...
            do
            {
                ...
                if (world.provider instanceof WorldProviderDarkAsteroids)
                {
                    bv3 = ((WorldProviderDarkAsteroids) world.provider).getClosestAsteroidXZ(x, 0, z);
                }
                if (bv3 != null)
                {
                    ...
                    MPLog.debug("Testing asteroid at x" + bv3.x + " y" + bv3.y + " z" + bv3.z);
            ...
            MPLog.error("Failed to find good large asteroid landing spot! Falling back to making a small one.");
            this.makeSmallLandingSpot(world, x, z);
            return new Vector3(x, 310, z);
        }
        MPLog.error("Null player when teleporting to Asteroids!");
    ...
    private boolean goodAsteroidEntry(World world, int x, int yorig, int z)
    {
        for (int k = 208; k > 48; k--)
        {
            if (!world.isAirBlock(x, k, z))
            {
                ...
                MPLog.debug("Found asteroid at x" + x + " z" + z);
                return true;
            }
        }
        return false;
